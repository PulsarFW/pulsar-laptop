...
function defaultApps()
	return {
		installed = config.defaultApps,
		home = config.defaultApps,
	}
end
...
CreateThread(function()
	...
	plsr.Inventory.Items:RegisterUse("laptop", "Laptop", function(source, itemData)
		TriggerClientEvent("Laptop:Client:Open", source)
	end)

	plsr.Reputation:Create("Chopping", "Vehicle Chopping", {
	...
	plsr.Reputation:Create("Boosting", "Boosting", {
		{ label = "D", value = 0 },
		{ label = "C", value = 6000 },
		{ label = "B", value = 15000 },
		{ label = "A", value = 50000 },
...
AddEventHandler("Laptop:Server:RegisterMiddleware", function()
	...
	plsr.Middleware:Add("Laptop:UIReset", function(source)
		plsr.Laptop:UpdateJobData(source)
		TriggerClientEvent("Laptop:Client:SetApps", source, LAPTOP_APPS)
	end)
	plsr.Middleware:Add("Characters:Creating", function(source, cData)
		local t = plsr.Middleware:TriggerEventWithData("Laptop:CharacterCreated", source, cData)
...
RegisterNetEvent("Laptop:Server:UIReset", function()
	plsr.Middleware:TriggerEvent("Laptop:UIReset", source)
end)

AddEventHandler("Laptop:Server:RegisterCallbacks", function()
	plsr.Callbacks:RegisterServerCallback("Laptop:Permissions", function(src, data, cb)
