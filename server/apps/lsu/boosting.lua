local config = load(LoadResourceFile(GetCurrentResourceName(), "config/server.lua"))().lsu.boosting
...
AddEventHandler("Laptop:Server:RegisterCallbacks", function()
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:Admin:CreateContract", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char and data?.vehicle and data?.make and data?.model and data?.trackers and data?.price then
            ...
            if perm["lsunderground"] and perm["lsunderground"]["admin"] then
                ...
                for k, v in ipairs(config.repToClass) do
                    if v == data.class then
                        level = k
                    end
                end

                if level >= 1 and config.vehicleConfig[data.class] and cost > 0 then
                    local category = config.vehicleConfig[data.class]
                    local length = math.random(category.length.min, category.length.max) * 60 * 60
                    local tracker = tonumber(data.trackers) or 0
        
                    cb(plsr.Laptop.LSUnderground.Boosting:GiveContract(source, {
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:Admin:GetBans", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char then
            local perm = char:GetData("LaptopPermissions")
            ...
            if perm["lsunderground"] and perm["lsunderground"]["admin"] then
                plsr.Database:Query(
                    "SELECT `id`, `data` FROM `characters` WHERE JSON_CONTAINS_PATH(`data`, 'one', '$.LSUNDGBan') AND `deleted` = 0",
                    nil,
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:Admin:Ban", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:Admin:Unban", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char and data?.SID then
            local perm = char:GetData("LaptopPermissions")
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:EnterQueue", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        ...
        for k,v in ipairs(team.Members) do
            local reputation = plsr.Reputation:GetLevel(v.Source, config.requiredRep.rep)
            if not reputation or reputation < config.requiredRep.level then
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:ExitQueue", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char then
            for k, v in ipairs(_boostingQueue) do
                if v.source == source then
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:DeclineContract", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char then
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:TransferContract", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char and data?.alias then
            ...
            for k, v in ipairs(boostingContracts) do
                if v.id == data.id then
                    local found = nil
                    for _, c in pairs(plsr.Fetch:AllCharacters()) do
                        if c ~= nil then
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:AcceptContract", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char then
            local boostingContracts = char:GetData("BoostingContracts") or {}
            local perm = char:GetData("LaptopPermissions")
            local team, isLeader = plsr.Laptop.Teams:GetByMemberSource(source)
            local level = plsr.Reputation:GetLevel(source, "Boosting")
            if level < 1 then
                level = 1
            end
            ...
            for k, v in ipairs(boostingContracts) do
                if v.id == data.id and (v.expires > os.time()) and (level >= v.vehicle.classLevel or v.vehicle.rewarded) then
                    if team?.ID and (#team.Members >= 2 or (perm["lsunderground"] and perm["lsunderground"]["admin"])) then
                        ...
                        if plsr.Crypto:Has(source, v.prices.standard.coin, v.prices.standard.price) then
                            ...
                            plsr.Laptop.Notification:Add(team.ID, "New Request for a Boosting Contract", string.format("%s %s Requested a Boosting Contract (%s)", char:GetData("First"), char:GetData("Last"), v.vehicle.class), os.time() * 1000, 15000, "lsunderground", {
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:Exterior", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char then
            ...
            if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 0 then
                ...
                local makeDifficult = (GlobalState["Duty:police"] or 0) < config.policeDifficult
                local classData = config.vehicleConfig[_boosting[team.ID].vehicleData?.class]
                ...
                plsr.Laptop.Teams.Members:SendEvent(team.ID, "Laptop:Client:LSUnderground:Boosting:UpdateState", 1)
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:Ignition", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char then
            local team = plsr.Laptop.Teams:GetByMemberSource(source)
            ...
            if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 1 then
                ...
                if _boosting[team.ID].trackerTotal and _boosting[team.ID].trackerTotal > 0 then
                    ...
                    plsr.Laptop.Teams.Members:SendEvent(team.ID, "Laptop:Client:LSUnderground:Boosting:UpdateState", 2)
                    plsr.Laptop.Teams.Members:NotificationAddWithId(
                        team.ID,
                        "BOOSTING_CONTRACT",
                        "Current Contract",
                    ...
                    plsr.Laptop.Teams.Members:SendEvent(team.ID, "Laptop:Client:LSUnderground:Boosting:UpdateState", 3)
                end
            end
        end
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:DropOff", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char then
            local team = plsr.Laptop.Teams:GetByMemberSource(source)
            ...
            if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 3 then
                ...
                plsr.Laptop.Teams.Members:SendEvent(team.ID, "Laptop:Client:LSUnderground:Boosting:UpdateState", 4)
            end
        end
    ...
    plsr.Callbacks:RegisterServerCallback("Laptop:LSUnderground:Boosting:LeftArea", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
        if char then
            local team = plsr.Laptop.Teams:GetByMemberSource(source)
            ...
            if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 4 then
                ...
                if not stillInZone then
                    if #(GetEntityCoords(_boosting[team.ID].vehicle) - _boosting[team.ID].dropOff) <= 30.0 then
                        plsr.Laptop.LSUnderground.Boosting:Complete(team?.ID)
                    else
                        plsr.Laptop.Teams.Members:Notification(
                            team.ID,
                            "Current Contract",
    ...
    plsr.Inventory.Items:RegisterUse("boosting_tracking_disabler", "Boosting", function(source, slot, itemData)
        local team = plsr.Laptop.Teams:GetByMemberSource(source)
        ...
        if team?.ID and _boosting[team.ID]
            ...
            and plsr.State.Entity(inVeh).boostVehicle == team.ID
        ...
        then
            Citizen.SetTimeout(500, function()
                plsr.Callbacks:ClientCallback(source, "Laptop:LSUnderground:Boosting:TrackerHacker", {}, function(using, success)
            ...
            plsr.Execute:Client(source, "Notification", "Error", "Can't Use Right Now...")
        end
	end)
...
AddEventHandler("Laptop:Server:LSUnderground:Boosting:ActionRequest", function(source, data, action)
    if action == "accept" and data?.requester and data?.contract then
        local owner = plsr.Fetch:CharacterSource(data.requester)
        if not owner then return; end

        local team, isLeader = plsr.Laptop.Teams:GetByMemberSource(data.requester)
        ...
        for k, v in ipairs(contracts) do
            if v.id == data.contract then
                ...
                if team?.ID and team.State == 0 and (#team.Members >= 2 or (perm["lsunderground"] and perm["lsunderground"]["admin"])) then
                    ...
                    local fail = false
                    for k, v in ipairs(team.Members) do
                        local char = plsr.Fetch:CharacterSource(v.Source)
                        if char then
                    ...
                    if not fail then
                        if plsr.Crypto.Exchange:Remove(v.prices.standard.coin, owner:GetData("CryptoWallet"), v.prices.standard.price) then
                            table.remove(contracts, k)
                            updated = true
    
                            plsr.Laptop.LSUnderground.Boosting:Start(team.ID, v)
                        else
                            for _, m in ipairs(team.Members) do
                                plsr.Laptop.Notification:Add(m.Source, "Failed to Start Boosting Contract", string.format("%s %s doesn't have enough crypto to start the contract.", owner:GetData("First"), owner:GetData("Last")), os.time() * 1000, 10000, "lsunderground", {}, {})
                            end
                        end
                    else
                        plsr.Laptop.Teams.Members:Notification(
...
LAPTOP.LSUnderground.Boosting = {
    RewardContract = function(self, source)
        ...
        if char then
            ...
            local boostingClass = plsr.Utils:WeightedRandom(config.repToClassChances[level])

            if not config.vehicleConfig[boostingClass] then
                return false
            end

            local category = config.vehicleConfig[boostingClass]
            local isRare = false
            local length = math.random(category.length.min, category.length.max) * 60 * 60
            local veh, cost
            ...
            return plsr.Laptop.LSUnderground.Boosting:GiveContract(source, {
                vehicle = veh.vehicle,
    ...
    GiveContract = function(self, source, vehicle, prices, timeLength, settings)
        ...
        if char then
            ...
            plsr.Logger:Info("Boosting", string.format("%s [%s %s (%s)] Rewarded Class %s Contract (%s)%s", alias, char:GetData("First"), char:GetData("Last"), char:GetData("SID"), vehicle.class, vehicle.label, vehicle.rewarded and " (Manually Created)" or ""))
    ...
    Start = function(self, teamId, contract)
        local team = plsr.Laptop.Teams:Get(teamId)
        if team and contract and contract?.vehicle?.vehicle then
            local dropOffLocation = config.dropoffs[math.random(#config.dropoffs)]

            if _boostingEvent then
                dropOffLocation = config.eventDropoff
            end
            ...
            for k, v in ipairs(team.Members) do
                plsr.Reputation.Modify:Remove(v.Source, "Boosting", takenRep)

                if v.SID == contract.owner?.SID then
                    local cChar = plsr.Fetch:CharacterSource(v.Source)
                    if cChar then
                        plsr.Logger:Info("Boosting", string.format("%s [%s %s (%s)] Started Class %s Contract (%s)", cChar:GetData("Profiles")?.redline?.name, cChar:GetData("First"), cChar:GetData("Last"), cChar:GetData("SID"), contract.vehicle.class, contract.vehicle.label))
                    end
                end
            end
            ...
            plsr.Laptop.Teams:SetState(team.ID, "boosting", string.format("On %s Boosting Contract", contract.vehicle.class))
            ...
            plsr.Vehicles:SpawnTemp(source, GetHashKey(contract.vehicle.vehicle), 'automobile', pickUpLocation.coords.xyz, pickUpLocation.coords.w, function(spawnedVehicle, VIN, plate)
                if spawnedVehicle then
                    ...
                    plsr.Laptop.Teams.Members:SendEvent(team.ID, "Laptop:Client:LSUnderground:Boosting:Start", _boosting[team.ID])
                    ...
                    plsr.Logger:Error("Boosting", string.format("Failed to Spawn Vehicle For Boost. Contract Owner SID: %s. Buy In: %s $%s", contract.owner.SID, contract.prices.standard.price, contract.prices.standard.coin))
                end
            end, {
    ...
    Cancel = function(self, teamId, teamDeleted)
        if _boosting[teamId] then
            if not teamDeleted then
                plsr.Laptop.Teams:ResetState(teamId)
                plsr.Laptop.Teams.Members:NotificationRemoveById(teamId, "BOOSTING_CONTRACT")
                plsr.Laptop.Teams.Members:Notification(
                    teamId,
                    "Contract Cancelled",
                ...
                plsr.Laptop.Teams.Members:SendEvent(teamId, "Laptop:Client:LSUnderground:Boosting:End", true)
            end
        ...
        if _boosting[teamId] then
            ...
            if DoesEntityExist(veh) then
                ...
                for k, v in ipairs(_boosting[teamId].members) do
                    plsr.Reputation.Modify:Add(v.Source, "Boosting", math.floor(_boosting[teamId].takenRep + earnedRep))

                    if v.SID == _boosting[teamId].contractOwner?.SID then
                        local cChar = plsr.Fetch:CharacterSource(v.Source)
                        if cChar then
                            plsr.Crypto.Exchange:Add(
                                _boosting[teamId].coin, 
                                cChar:GetData("CryptoWallet"), 
                                math.floor(_boosting[teamId].price + earnedCrypto)
                            )
    
                            plsr.Logger:Info("Boosting", string.format("%s [%s %s (%s)] Completed Class %s Contract (%s)%s. Rep Gained: %s Crypto Gained: %s", cChar:GetData("Profiles")?.redline?.name, cChar:GetData("First"), cChar:GetData("Last"), cChar:GetData("SID"), _boosting[teamId].vehicleData.class, _boosting[teamId].vehicleData.label, _boosting[teamId].vehicleData.rewarded and " (Manually Created)" or "", earnedRep, earnedCrypto))
                        end
                    end
                end
            ...
            plsr.Laptop.Teams:ResetState(teamId)
            plsr.Laptop.Teams.Members:NotificationRemoveById(teamId, "BOOSTING_CONTRACT")
            plsr.Laptop.Teams.Members:SendEvent(teamId, "Laptop:Client:LSUnderground:Boosting:End")
            TriggerClientEvent("EmergencyAlerts:Client:TrackerBlip", -1, "police", string.format("boosting-%s", teamId))
            _boosting[teamId] = nil
        end
    end,
...
AddEventHandler("Laptop:Server:Teams:MemberRemoved", function(teamId, member)
    ...
    if team and team.Members and #team.Members < 2 then -- No longer enough people, kick 'em out of the queue
        ...
        if _boosting[teamId] then
            plsr.Laptop.LSUnderground.Boosting:Cancel(teamId)
        end
    end
end)
...
AddEventHandler("Laptop:Server:Teams:Deleted", function(teamId)
    ...
    if _boosting[teamId] then
        plsr.Laptop.LSUnderground.Boosting:Cancel(teamId, true)
    end
end)
...
AddEventHandler("Vehicles:Server:Deleted", function(veh)
    for k, v in pairs(_boosting) do
        if v and v.vehicle == veh then
            plsr.Laptop.LSUnderground.Boosting:Cancel(k)
        end
    end
end)
...
function SetupBoostingQueue()
    ...
    CreateThread(function()
        Wait(config.serverStartWait)
        plsr.Logger:Info("Boosting", "Boosting Contracts Can Now Be Rewarded")
        ...
        while true do
            ...
            if #_boostingQueue > 0 then
                --local index = math.random(#_boostingQueue + (makeDifficult and config.randomnessDifficult or config.randomness))

                local index = math.random(
                    #_boostingQueue - (makeDifficult and config.randomnessDifficult or config.randomness),
                    #_boostingQueue + (makeDifficult and config.randomnessDifficult or config.randomness)
                )

                if _boostingQueue[index] then
                    local char = plsr.Fetch:CharacterSource(_boostingQueue[index].source)
                    if char then
                        ...
                        if not _boosting[_boostingQueue[index].team] and (not _boostingContractCount[char:GetData("SID")] or (_boostingContractCount[char:GetData("SID")] < config.maxContracts)) and (contractCount < config.maxQueue) then
                            chosen = true

                            plsr.Laptop.LSUnderground.Boosting:RewardContract(char:GetData("Source"))
                        end
                    end
                end

                if not chosen then
                    plsr.Logger:Info("Boosting", string.format("Nobody Chosen for Boosting Contract (%s | %s)", index, #_boostingQueue))
                end
            end

            Wait((1000 * 60) * math.random(config.timeBetweenMin, config.timeBetweenMax))
        end
    end)
end
