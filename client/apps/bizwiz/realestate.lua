...
RegisterNUICallback("Dyn8SearchProperties", function(d, cb)
    ...
    cb({
        properties = data,
        upgrades = plsr.Properties:GetUpgradesConfig(),
    })
end)
...
RegisterNUICallback("Dyn8StartSale", function(data, cb)
	plsr.Callbacks:ServerCallback("Properties:Dyn8:Sell", {
...
RegisterNUICallback("Dyn8StartTransfer", function(data, cb)
	plsr.Callbacks:ServerCallback("Properties:Dyn8:Transfer", {
        property = data.property,
        target = data.SID,
    }, cb)
end)
...
RegisterNUICallback("Dyn8ChangePropertyLocations", function(data, cb)
    plsr.Callbacks:ServerCallback("Properties:EditProperty", data, function(success)
...
RegisterNUICallback("Dyn8ShowPropertyLocations", function(data, cb)
    local prop = plsr.Properties:Get(data.property)
    if prop and prop.location then
        ...
        CreateThread(function()
            while plsr.State.flags.loggedIn and _drawingPropertyLocations do
